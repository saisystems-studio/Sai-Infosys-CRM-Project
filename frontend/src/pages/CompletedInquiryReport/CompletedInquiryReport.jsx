...
import { exportCompletedInquiryReport } from "./completedInquiryReportExport";
...
const productName = (item) =>
  item.product_name || item.product_type_name || "Product";
const isAmcInquiry = (inquiry) =>
  (inquiry.products || []).some((product) => product.is_amc);
export default function CompletedInquiryReport({ onViewDetails }) {
  ...
  const loadReport = async () => {
    ...
    try {
      const response = await axios.get(
        `${API_BASE_URL}/inquiries/completed-inquiry-report/`,
        {
          headers: {
      ...
      setError(
        requestError.response?.data?.detail ||
          "Unable to load the completed inquiry report.",
      );
    } finally {
      setLoading(false);
    }
  ...
  useEffect(() => {
    let active = true;
    axios
      .get(`${API_BASE_URL}/inquiries/completed-inquiry-report/`, {
        headers: {
          Authorization: `Bearer ${localStorage.getItem("crm_access_token")}`,
        },
      })
  ...
  const staffOptions = [
    ...new Map(
      rows
        ...
        .map((row) => [String(row.Resource_Id), row.resource_name]),
    ).entries(),
  ];
  const productOptions = [
    ...new Set(rows.flatMap((row) => (row.products || []).map(productName))),
  ].sort();
  ...
  const exportRows = reportRows.map(({ inquiry, task }) => [
    `#${inquiry.id}`,
    inquiry.company_name || "No company name",
    inquiry.status_name || "Completed",
    isAmcInquiry(inquiry) ? "AMC" : "-",
    task.resource_name || inquiry.resource_name || "Unassigned",
    getCompactCompletedDateTime(task.end_time),
    task.progress_notes || "No notes recorded",
    formatDuration(task),
    (inquiry.products || []).map(productName).join(", ") || "Not specified",
  ]);
  const handleExport = async (format) => {
    setExporting(format);
    try {
      await exportCompletedInquiryReport(
        format,
        exportRows,
        datePreset === "custom"
          ? `${filters.fromDate} to ${filters.toDate}`
          : datePreset.replaceAll("-", " "),
      );
    } finally {
      setExporting("");
    }
  ...
  if (loading)
    return (
      <div className="completed-report-state">
        ...
        Loading today’s completed inquiries…
      </div>
    );
    ...
    <section className="completed-report-page">
      <header className="completed-report-header">
        <div>
          ...
          <p>
            Latest completed-task update, including payment-pending work, for
            each inquiry in the selected period.
          </p>
        </div>
        <div className="completed-report-total">
          <strong>{reportRows.length}</strong>
          <span>Completed task inquiries</span>
        </div>
      </header>
      ...
      <div className="completed-report-filters">
        <label className="completed-report-search">
          ...
          <input
            ...
            placeholder="Company, staff, product or task…"
            onChange={(event) =>
        ...
        <label>
          ...
          <select
            ...
            onChange={(event) =>
              setFilters({ ...filters, staffId: event.target.value })
            }
          >
        ...
        <div className="completed-report-export-actions">
          <button
            type="button"
            disabled={!reportRows.length || Boolean(exporting)}
      ...
      {error ? (
        ...
        <div className="completed-report-empty">
          <strong>No completed inquiries found</strong>
          <span>Try choosing a different period or adjusting the filters.</span>
        </div>
      ) : (
        <div className="completed-report-table-wrap">
          <div
            className="completed-report-table"
            role="table"
            aria-label="Completed inquiry details"
          >
            ...
            {reportRows.map(({ inquiry, task }) => (
              <div
                className="completed-report-table-row"
                role="row"
                key={inquiry.id}
              >
                <span role="cell" data-label="Inquiry">
                  #{inquiry.id}
                </span>
                <span
                  role="cell"
                  data-label="Company"
                  className="completed-report-customer-cell"
                >
                  <strong>{inquiry.company_name || "No company name"}</strong>
                </span>
                <span role="cell" data-label="Status">
                  <em
                    className={`completed-report-status ${inquiry.status_name === "Payment Pending" ? "completed-report-status-pending" : ""}`}
                  >
                    {inquiry.status_name || "Completed"}
                  </em>
                </span>
                <span role="cell" data-label="Type">
                  {isAmcInquiry(inquiry) ? (
                    <em className="completed-report-amc">AMC</em>
                  ) : (
                    "—"
                  )}
                </span>
                <span role="cell" data-label="Staff">
                  {task.resource_name || inquiry.resource_name || "Unassigned"}
                </span>
                ...
                <span
                  ...
                  className="completed-report-notes"
                >
                  {task.progress_notes || "No notes recorded"}
                </span>
                <span
                  role="cell"
                  data-label="Duration"
                  className="completed-report-duration"
                >
                  {formatDuration(task)}
                </span>
                <span
                  role="cell"
                  data-label="Products"
                  className="completed-report-products-cell"
                >
                  {(inquiry.products || []).length
                    ? inquiry.products.map((item) => (
                        <em key={item.id}>{productName(item)}</em>
                      ))
                    : "Not specified"}
                </span>
                <span role="cell" data-label="View">
                  <button
                    type="button"
                    className="completed-report-view-icon"
                    onClick={() => onViewDetails(inquiry.id)}
                    aria-label={`View full details for inquiry ${inquiry.id}`}
